...
class ChoiceSchoolsController < ApplicationController
  ...
  before_action :redirect_if_student_token_invalid, only: %i[verify confirmation]
  ...
  def rank
    ...
    if params[:schools].blank? || params[:schools].values.all?(&:blank?)
      ...
      properly_formatted = begin
        rankings.map { |x| x.try(:to_i) }.sort == (rankings.map do |x|
          x.try(:to_i)
