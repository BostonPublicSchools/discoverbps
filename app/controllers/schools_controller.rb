...
class SchoolsController < ApplicationController
  ...
  before_action :load_schools, except: [:print]
  ...
  def index
    if current_user_students.blank?
      ...
      if @home_schools.blank?
        ...
        @home_school_ids = @home_schools.collect do |x|
          sql_helper_group_schoolId(x.school.bps_id, current_student.grade_level)
        end.join(',')
        looker_studio_links
  ...
  def compare
    if current_user_students.blank?
      ...
      @matching_school_ids = current_user_students.collect { |x| x.student_schools.collect(&:bps_id) }.inject(:&)
      if @home_schools.blank?
        flash[:alert] = 'There were no schools that matched your search. Please try again.'
        redirect_to root_url
      else
  ...
  def load_schools
    if current_student.present?
      ...
      if current_student.student_schools.collect(&:ranked).any?
