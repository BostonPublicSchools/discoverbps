...
module SchoolsHelper
  ...
  def student_support_list_helper(school)
    ...
    if student_support.present? || facilities.present?
      array = []
      array << "Family Coordinator"	if student_support.present? && student_support.try(:[], :HasFamilyCoord) == "True"
      array << "Full-Time Nurse"	if student_support.present? && student_support.try(:[], :HasFullTimeNurse) == "True"
      array << "Guidance Counselor"	if student_support.present? && student_support.try(:[],
        :HasGuidanceCoord) == "True"
      array << "Handicap Access"	if facilities.present? && facilities.try(:[], :HandicappedAccess) == "T"
      array << "Online Health Center"	if student_support.present? && student_support.try(:[],
        :HasOnlineHealthCntr) == "True"
      array << "Part-Time Nurse"	if student_support.present? && student_support.try(:[], :HasPartTimeNurse) == "True"
      array << "Social Worker"	if student_support.present? && student_support.try(:[], :HasSocialWorker) == "True"
      array.compact
    else
      []
    end
  ...
  def school_hours_end_time_helper(hours)
    ...
    hour	= hours.gsub("am", "").gsub("pm", "").match(/-\s?\d*:/).to_s.gsub(/-\s?/, "").delete(":").to_s.to_i
    minutes = hours.gsub("am", "").gsub("pm", "").match(/-\s?\d*:\d*/).to_s.gsub(/-\s?\d*:/, "").to_s.to_i
    hour += 12 if hour.positive? && hour < 8 # add 12 if it's pm
    decimal = hour + (minutes / 60.0)
  ...
  def special_admissions_helper(string)
    if string.blank? || string == "False" || string == "false" || string == false
      "No"
    elsif ["True", "true", true].include?(string)
      "Yes"
    end
  end
  ...
  def sql_helper_group_schoolId(schoolId, grade)
    if schoolId.present?
      if schoolId == "4241"
        ...
        schoolId = "‘1195’"
      elsif %w[4391 1140].include?(schoolId)
        schoolId = if %w[09 10 11 12].include?(grade)
          "1140"
        else
          "4391"
        end
