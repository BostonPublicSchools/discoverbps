...
class School < ApplicationRecord
  ...
  def applicants(grade_level, school_year = nil)
    ...
    if year.present?
      ...
      demand_data.where(grade_level: grade).average(:total_applicants).try(:to_i)
    end
  end
  ...
  def open_seats(grade_level, school_year = nil)
    ...
    if year.present?
      ...
      demand_data.where(grade_level: grade).average(:seats_before_round).try(:to_i)
    end
  end
  ...
  def applicants_per_open_seat(grade_level, school_year = nil)
    ...
    if grade.present?
      ...
      demand_data.where(year: year).last.try(:applicants_per_open_seat).try(:round, 2)
    elsif grade.present? && year.present?
      demand_data.where(grade_level: grade, year: year).last.try(:applicants_per_open_seat).try(:round, 2)
    end
  end
  ...
  def total_seats(grade_level, school_year = nil)
    ...
    if year.present?
      ...
      demand_data.where(grade_level: grade).average(:total_seats).try(:to_i)
    end
  end
  ...
  def sync_school_data_callback
    School.delay(priority: 5).sync_school_data!(id)
  end
end
