...
class PreferenceCategory < ApplicationRecord
  ...
  # attr_accessible :name, :description, :sort_order, :qualitative_criteria, :include_in_special_needs_dialog_box,
  #                 :include_in_preferences_panel, :select_type, :glyph_id, :glyph_class
  ...
  def active_preferences(grade)
    grade_level = grade.try(:downcase).try(:strip)
    Preference.where("preference_category_id = ? AND grade_#{grade_level} = ?", id, true).rank(:sort_order)
  end
end
