...
class StudentSchool < ApplicationRecord
  ...
  def self.create_from_api_response(student, school, school_hash, school_list_type)
    ...
    create!(student_id: student.id,
      ...
      bps_id: (if school_list_type == "ell"
                 school_hash[:SchoolId]
               else
                 (school_list_type == "sped") ? school_hash[:SchoolID] : school_hash[:SchoolLocalId]
               end),
      ...
      exam_school: school_hash[:IsExamSchool] != "0",
      ...
      program_code: (if school_list_type == "ell"
                       school_hash[:ProgramCode]
                     else
                       (school_list_type == "sped") ? school_hash[:MidCode] : school_hash[:ProgramId]
                     end),
